      ...
      <div v-for="category in filteredCatalog" :key="category.id" class="space-y-5">
        ...
        <div v-else class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          <article
            ...
            :key="product.id"
            class="group flex flex-col overflow-hidden rounded-[24px] border border-zinc-200 bg-white transition-all hover:shadow-lg dark:border-zinc-800 dark:bg-zinc-950"
          >
            <!-- Image / Status -->
            <div class="relative aspect-[4/3] w-full overflow-hidden bg-[linear-gradient(135deg,#fafafa,#e4e4e7)] dark:bg-[linear-gradient(135deg,#27272a,#09090b)]">
              <img v-if="product.imageUrl" :src="product.imageUrl" :alt="product.name" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
              <div v-else class="flex h-full flex-col items-center justify-center gap-2 text-zinc-400">
                 <svg class="h-8 w-8 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                 <span class="text-[11px] font-medium tracking-wide">Sin foto</span>
              </div>
              
              <div class="absolute inset-x-3 top-3 flex items-start justify-between gap-2">
                 <span v-if="product.hasPromo" class="rounded-full bg-orange-600/95 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow-sm backdrop-blur-sm">
                   Promo
                 </span>
                 <span v-else></span>
                 
                 <span class="rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-sm transition-colors" :class="product.isActive ? 'bg-zinc-900/90 text-white dark:bg-white/90 dark:text-zinc-900' : 'bg-white/90 text-zinc-600 dark:bg-black/80 dark:text-zinc-400'">
            ...
            <div class="flex flex-1 flex-col p-5">
              <h4 class="text-[17px] font-bold text-zinc-900 dark:text-zinc-100">{{ product.name }}</h4>
              <p class="mt-1.5 line-clamp-2 text-[13px] leading-relaxed text-zinc-500 dark:text-zinc-400">{{ product.description || 'Sin descripción detallada' }}</p>
              
              <div class="mt-auto pt-5">
                 <div class="flex items-end justify-between">
                   <div class="space-y-0.5">
                     <span v-if="product.hasPromo && product.promoPrice !== null" class="block text-[11px] font-semibold tracking-wide text-orange-600 line-through opacity-70">{{ money(product.basePrice) }}</span>
                     <span class="block text-xl font-black tracking-tight" :class="product.hasPromo ? 'text-orange-600' : 'text-zinc-900 dark:text-zinc-100'">
                       {{ money(product.hasPromo ? (product.promoPrice || 0) : product.basePrice) }}
                     </span>
                   </div>
                   <span class="rounded-full bg-zinc-100 px-2 py-1 text-[10px] font-bold tracking-widest text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                     {{ product.variants.length ? `+${product.variants.length} OP.` : 'BASE' }}
                   </span>
                 </div>
                 
                 <div class="mt-4 grid grid-cols-2 gap-2">
                   <button class="ghost-btn small !rounded-xl !bg-zinc-50 !py-2.5 text-xs font-semibold hover:!bg-zinc-100 dark:!bg-zinc-900/50 dark:hover:!bg-zinc-800" @click="openProductEditor(product)">Modificar</button>
                   <button class="ghost-btn small !rounded-xl !bg-zinc-50 !py-2.5 text-xs font-semibold !text-red-500 hover:!bg-red-50 dark:!bg-zinc-900/50 dark:hover:!bg-red-950/30" @click="removeProduct(product)">Eliminar</button>
                 </div>
              </div>
            </div>
          </article>
